    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl lg:min-h-screen" ref={pinRef}>
        <div className="grid gap-10 lg:min-h-screen lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#9fb8ff]">
              Production Support
            </p>
              ...
              {serviceCards.map((service, index) => (
                <button
                  className={cn(
                    ...
                    index === activeIndex
                      ? "running-light-card bg-white shadow-[0_18px_60px_rgba(47,109,246,0.18)] dark:bg-white/12"
                      : "border-[#0f172c]/10 bg-white/70 shadow-sm hover:border-[#2f6df6]/35 hover:bg-white dark:border-white/10 dark:bg-white/[0.04] dark:hover:border-white/25 dark:hover:bg-white/[0.07]"
                  )}
                  key={service.title}
                  onClick={() => setActiveIndex(index)}
                  type="button"
                >
                  <span className="relative z-10 text-xs font-black uppercase tracking-[0.2em] text-[#2f6df6] dark:text-[#9fb8ff]">
                    {service.eyebrow}
                  </span>
            ...
            <div className="relative min-h-[520px] overflow-hidden rounded-[32px] border border-[#0f172c]/10 bg-white shadow-[0_30px_100px_rgba(15,23,44,0.18)] dark:border-white/10 dark:bg-white/[0.05] dark:shadow-[0_30px_100px_rgba(0,0,0,0.35)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
                <div className="max-w-2xl rounded-3xl border border-white/18 bg-black/42 p-5 text-white shadow-[0_18px_70px_rgba(0,0,0,0.22)] backdrop-blur-xl">
                  ...
                  <Link
                    className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#2f6df6] px-5 py-3 text-sm font-black text-white transition hover:-translate-y-0.5 hover:bg-[#5b8cff]"
                    href={active.href}
                  >
                    Explore Service
